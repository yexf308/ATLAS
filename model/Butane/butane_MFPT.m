       ...
       t_final                            = zeros(3,10);
       dt_s                               = params.RHS.t0;
       Butane_analysis_fileName           = [params.paths.datapath,'Butane_analysis.mat'];
        ...
        [FPT_ori1,exit_curr1_ori, t_final_ori(1)] = MFPT_butane2_ori(well_threshold1, output, params.MFPT.N_IC, params.RHS,params.chart_sim_parameter);
        [FPT_ori2,exit_curr2_ori, t_final_ori(2)] = MFPT_butane2_ori(well_threshold2, output, params.MFPT.N_IC, params.RHS,params.chart_sim_parameter);
        [FPT_ori3,exit_curr3_ori, t_final_ori(3)] = MFPT_butane2_ori(well_threshold3, output, params.MFPT.N_IC, params.RHS,params.chart_sim_parameter);

    for k = 1:10
        disp([num2str(k),'th round'])
        chart_fileName                     = [params.paths.datapath,'chart',num2str(k),'.mat'];
        FPT_fileName                       = [params.paths.datapath,'Butane_FPT',num2str(k),'.mat'];
        load( chart_fileName )
        ...
        [FPT_sim1,exit_curr1_sim,t_final(1,k)] = MFPT_butane2_sim(weighted_dd2, well_threshold1, output, params.MFPT.N_IC, phi_store, chart,params.chart_sim_parameter);
        [FPT_sim2,exit_curr2_sim,t_final(2,k)] = MFPT_butane2_sim(weighted_dd2, well_threshold2, output, params.MFPT.N_IC, phi_store, chart,params.chart_sim_parameter);
        [FPT_sim3,exit_curr3_sim,t_final(1,k)] = MFPT_butane2_sim(weighted_dd2, well_threshold3, output, params.MFPT.N_IC, phi_store, chart,params.chart_sim_parameter);
        ...
        relative_error_FPT(3, k) = (Mean_FPT(6,k)-Mean_FPT(3,k))/Mean_FPT(3,k);   
         ...
         Q1(1,2,k) = sum(exit_curr1_ori>0)/params.MFPT.N_IC * (-Q1(1,1,k));
         Q1(1,3,k) = sum(exit_curr1_ori<0)/params.MFPT.N_IC * (-Q1(1,1,k));
         Q1(2,1,k) = sum(exit_curr2_ori>0)/params.MFPT.N_IC * (-Q1(2,2,k));
         Q1(2,3,k) = sum(exit_curr2_ori<0)/params.MFPT.N_IC * (-Q1(2,2,k));
         Q1(3,1,k) = sum(exit_curr3_ori<0)/params.MFPT.N_IC * (-Q1(3,3,k));
         Q1(3,2,k) = sum(exit_curr3_ori>0)/params.MFPT.N_IC * (-Q1(3,3,k));


         Q2(1,1,k) = -1/mean(FPT_sim1);
         Q2(2,2,k) = -1/mean(FPT_sim2);
         Q2(3,3,k) = -1/mean(FPT_sim3);
         Q2(1,2,k) = sum(exit_curr1_sim>0)/params.MFPT.N_IC * (-Q2(1,1,k));
         Q2(1,3,k) = sum(exit_curr1_sim<0)/params.MFPT.N_IC * (-Q2(1,1,k));
         Q2(2,1,k) = sum(exit_curr2_sim>0)/params.MFPT.N_IC * (-Q2(2,2,k));
         Q2(2,3,k) = sum(exit_curr2_sim<0)/params.MFPT.N_IC * (-Q2(2,2,k));
         Q2(3,1,k) = sum(exit_curr3_sim<0)/params.MFPT.N_IC * (-Q2(3,3,k));
         Q2(3,2,k) = sum(exit_curr3_sim>0)/params.MFPT.N_IC * (-Q2(3,3,k));


        if k ==1
            disp(['The original simulator at trans state has ',      num2str(mean(FPT_ori1),'%5.4f'),'+-',num2str(std(FPT_ori1)*1.96/sqrt(params.MFPT.N_IC),'%5.4f')])
            disp(['The original simulator at cis-top state has ',    num2str(mean(FPT_ori2),'%5.4f'),'+-',num2str(std(FPT_ori2)*1.96/sqrt(params.MFPT.N_IC),'%5.4f')])
            disp(['The original simulator at cis-bottom state has ', num2str(mean(FPT_ori3),'%5.4f'),'+-',num2str(std(FPT_ori3)*1.96/sqrt(params.MFPT.N_IC),'%5.4f')])
            disp(['The ATLAS simulator at trans state has ',         num2str(mean(FPT_sim1),'%5.4f'),'+-',num2str(std(FPT_sim1)*1.96/sqrt(params.MFPT.N_IC),'%5.4f')])
            disp(['The ATLAS simulator at cis-top state has ',       num2str(mean(FPT_sim2),'%5.4f'),'+-',num2str(std(FPT_sim2)*1.96/sqrt(params.MFPT.N_IC),'%5.4f')])
            disp(['The ATLAS simulator at cis-bottom state has ',    num2str(mean(FPT_sim3),'%5.4f'),'+-',num2str(std(FPT_sim3)*1.96/sqrt(params.MFPT.N_IC),'%5.4f')])
