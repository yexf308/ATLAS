...
%SET_PARAMETER Configure the Peanut benchmark parameters and metadata.
%
%   params = SET_PARAMETER() returns a struct exposing the configuration that
%   used to be scattered in the base workspace. Scripts should reference the
%   returned struct to access settings, simulators, and file paths.

  datapath = evalin('base','datapath');

  D        = 3;                    % The dimension of the full system
  d        = 2;                    % The dimension of the slow manifold
  chi_p    = 5.991;                % 95% Confidence interval for 2D Gaussian distribution
  K_int    = 100;                  % Number of initial points on the manifold.
  N        = 400000;               % Number of short trajectories starting from the same initial point
  epsilon  = 0.005;                % Small parameter
  dt       = 5*10^-4;              % Simulation time-step
  T_max    = 0.1;                  % Time for short trajectories.
  t0       = 0.1;                  % The relaxation time.
  T_one    = 1000;                 % Time for single long trajectory to learn initial K charts.
  threshold = [1, 0.001];          % Thresholds for projection and occasional projection
  connectivity_threshold = 3;      % Two landmarks are connected if within r*sqrt(t0)
  ...
  LowerBound = 0.05;               % Lower bound of the training window
  UpperBound = 0.10;               % Upper bound of the training window
  relearn_option     = 0;
  ...
  % Set up an initial point to start
  theta_int               = rand * pi;
  phi_int                 = rand * 2*pi;
  R_int                   = R(theta_int, phi_int);
  X_int                   = [R_int*sin(theta_int)*cos(phi_int), R_int*sin(theta_int)*sin(phi_int), R_int*cos(theta_int) ];

  % Specify the drift and diffusion in spherical coordinate
  r_RHS           = @(r, theta, phi) -(c1./epsilon)./r.*( r - R(theta,phi) );
  theta_RHS       = @(r, theta, phi)   c3* cos(3*theta)./(r.*sin(theta)) ;
  phi_RHS         = @(r, theta, phi)   c5* sin(phi + theta)./r ;
  ...
  b_ito           = @(theta, phi) [ (1/2).*(a1+a2.*cos(theta).^2).^(-5/2).*cos(phi).*sin(theta).*((-1).*c6.^2.*(a1+a2.*cos(theta).^2).^2+(-1).*c4.^2.*(a1+a2.*cos(theta).^2).*(a1+4.*a2.*cos(theta).^2).*sin(theta).^2+a1.*a2.*c4.^2.*sin(theta).^4);
                                    (-1/2).*(a1+a2.*cos(theta).^2).^(-5/2).*sin(theta).*(c6.^2.*(a1+a2.*cos(theta).^2).^2+c4.^2.*(a1+a2.*cos(theta).^2).*(a1+4.*a2.*cos(theta).^2).*sin(theta).^2+(-1).*a1.*a2.*c4.^2.*sin(theta).^4).*sin(phi);
                                    (-1/4).*c4.^2.*cos(theta).*(a1+a2.*cos(theta).^2).^(-5/2).*(2.*a1.^2+a2.^2+2.*a2.*(3.*a1+a2).*cos(2.*theta)+a2.^2.*cos(4.*theta)).*sin(theta).^2; ];
  ...
  params.exact = Exact_parameter;
  params.manifold = struct( ...
      'R', R, ...
      'Rprime', Rprime, ...
      'b_sph', b_sph, ...
      'Sigma_sph', Sigma_sph ...
